...
var require_macos_version = __commonJS({
  "node_modules/macos-version/index.js"(exports, module2) {
    "use strict";
    var fs = require("fs");
    ...
    module2.exports = getVersion;
    module2.exports.default = getVersion;
...
// generate-markdown-link.ts
var import_run = __toESM(require_run());
var originalClipboard = process.env.fakeClipboard;
(0, import_run.run)((originalClipboard2) => {
  ...
  function generate() {
    ...
    if (givenTitle) {
      ...
      console.log(`\u{1F7E1} \u{1F7E1} \u{1F7E1} delaying...`);
      delay(0.05);
      console.log(`\u{1F7E1} \u{1F7E1} \u{1F7E1} done delaying`);
  ...
  function parseBareDomain(url2) {
    const { parseDomain, fromUrl } = importParseDomain();
    const parseResult = parseDomain(fromUrl(url2));
  ...
  function copyChromeSelection({
    ...
    chrome2.copySelection(activeTab2);
    let selection = "";
    try {
      selection = app2.theClipboard();
    } catch (e) {
    ...
    const chromeHasSelection = !!selection && selection !== originalClipboard2;
  ...
  function normalizeSelection(selection) {
    return capitalizeFirstLetter(selection.trim().replace(/(?:\r\n|\r|\n)/g, " ").replace("Trusted Source", ""));
  }
  function importParseDomain() {
    ...
    var fromUrl = (urlLike) => {
      ...
      let url2 = urlLike.startsWith("//") ? `http:${urlLike}` : urlLike.startsWith("/") ? urlLike : urlPattern.test(urlLike) ? urlLike : `http://${urlLike}`;
    ...
    function extractHostname(url2) {
      var hostname;
      if (url2.indexOf("//") > -1) {
        hostname = url2.split("/")[2];
      } else {
        hostname = url2.split("/")[0];
      }
